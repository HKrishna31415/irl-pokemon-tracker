<script>
  ...
  let getActiveLevelCap = async () => 100
  let useRareCandy = () => {}
  ...
  onMount(() => {
    ...
    useRareCandy = async (monId, currentLevel) => {
      const qty = inventory['rare-candy'] || 0
      if (qty <= 0)
        return window.alert('You have no Rare Candies! Buy them in the Store.')
      ...
      if (isHard) {
        if (
          !window.confirm(
            `Use 1 Rare Candy to level up to Lv. ${+(currentLevel || 0) + 1}?`
          )
        )
          return
        ...
        if (lvl >= cap) {
          return window.alert(
            `This Pokemon is already at or above the current level cap (Lv. ${cap})!`
          )
        }
        if (
          !window.confirm(
            `Use 1 Rare Candy to level up to the level cap (Lv. ${cap})?`
          )
        )
          return
        gameStore.update(() => JSON.stringify(addTransaction({
  ...
  const formatPokemon = async (p) => {
    ...
    let moves =
      p.moves && p.moves.length
        ? p.moves.map((m) => (typeof m === 'string' ? m : m.name))
        : []
          ...
          const possibleMoves = data.moves
            .map((m) => {
              ...
              return levelUpDetail
                ? { name: m.move.name, level: levelUpDetail.level_learned_at }
                : null
            })
...
Ability: ${ability}
Level: ${level}
EVs: ${evs.hp || 0} HP / ${evs.atk || 0} Atk / ${evs.def || 0} Def / ${
      evs.spa || 0
    } SpA / ${evs.spd || 0} SpD / ${evs.spe || 0} Spe
${nature} Nature
IVs: ${ivs.hp} HP / ${ivs.atk} Atk / ${ivs.def} Def / ${ivs.spa} SpA / ${
      ivs.spd
    } SpD / ${ivs.spe} Spe
- ${moves[0]}
- ${moves[1]}
      ...
      >
        ...
        >
          ...
          <div class="flex flex-wrap items-center gap-2">
            <IconButton
              rounded
              title="Export Team to Showdown"
              on:click={exportToShowdown}
            >
        ...
        <div
          class="mt-4 flex items-center justify-between gap-x-4 border-b pb-2 dark:border-gray-700"
        >
          <h2 class="text-xl font-bold">Pokémon Box</h2>
          <div class="flex items-center gap-x-4">
            <div
              class="flex items-center gap-x-2 rounded-full border border-lime-100 bg-lime-50 px-3 py-1 text-sm dark:border-lime-900/30 dark:bg-lime-900/20"
            >
              <img
                ...
                class="h-4 w-4"
              />
              <span class="font-bold text-lime-700 dark:text-lime-400"
                >x{inventory['rare-candy'] || 0}</span
              >
            </div>
            <div
              class="font-mono text-lg font-bold text-lime-600 dark:text-lime-400"
            >
                  ...
                  >
                    ...
                    >
                      ...
                      <IconButton
                        ...
                        src="/assets/img/items/rare-candy.png"
                        title={isHardcore
                          ? 'Level Up by 1 (Use Rare Candy)'
                          : 'Level Up to Cap (Use Rare Candy)'}
                        disabled={!inventory['rare-candy']}
                        on:click={() =>
                          useRareCandy(p.customId || p.location, p.level)}
                      />
                      <IconButton
